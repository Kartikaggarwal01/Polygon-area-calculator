...
public class Start extends JPanel {
	...
	public Start(JFrame jFrame, int delay,int s) {
		...
		JLabel title[] = {new JLabel(" "),new JLabel("Polygon Area Calculator"),new JLabel(" ")};
		JLabel members[] = {new JLabel("Kartik Aggarwal - IIT2024068"),new JLabel(" "),new JLabel(" ")};
		...
		for(int i = 2; i< title.length;i++) {title[i].setAlignmentX(CENTER_ALIGNMENT);
		title[i].setAlignmentY(CENTER_ALIGNMENT);
		title[i].setFont(new Font("courier", Font.BOLD, 40));
		title[i].setForeground(drawPanel.c3);
		add(title[i]);}
		
		for(int i = 0; i< members.length;i++) {members[i].setAlignmentX(CENTER_ALIGNMENT);
		...
		// interpret delay in seconds; convert to milliseconds for Swing Timer
